...
module.exports = async (content, authorId) => {
	...
	for (var i = 0; i < splittedContent.length; i++) {
		const filteredSplittedContent = splittedContent[i].replace(
			/[^a-zA-Z0-9-@]/g,
			""
		);
		if (filteredSplittedContent.startsWith("@")) {
			const filteredMention = filteredSplittedContent.replace(
				/[^a-zA-Z0-9-]/g,
				""
			);

			try {
				// Check if the mention is a username
				const usernameData = await axios.get(
					`https://micro.alles.cx/api/username/${filteredMention}`
				);
				...
				// Ok it wasn't a username, hopefully it's an id
				const userById = await getUserById(filteredMention);
				if (userById !== undefined) mentionedIds.push(userById.id);
			}
		}
