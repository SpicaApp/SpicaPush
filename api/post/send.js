...
module.exports = async (req, res) => {
	...
	axios
		.post(
			"https://micro.alles.cx/api/posts/",
			{
				content: content,
				image: image,
				url: url,
		...
		.then(async (response) => {
			...
			var mentionedPeople = await getMentionedUsers(
			...
			for (var i = 0; i < authors.parents.length; i++) {
				var mentionedusers = await getMentionedUsers(
					authors.parents[i].content,
					authors.parents[i].author.id
				);
				...
				mentionedPeopleInParents = mentionedPeopleInParents.concat(
					mentionedusers
				);
			}
			...
			const subscribedPeople =
				parent === null || parent === undefined || parent === ""
					? await getSubscribedUsers(authors.post.author.id)
					: [];

			for (var i = 0; i < mentionedPeople.length; i++) {
				// Remove mentioned people from authors (so they don't get a notification twice - they only get one for mentions then)
				...
				const idIndexMentionedInParents = mentionedPeopleInParents.indexOf(
					mentionedPeople[i]
				);
			...
			await sendNotification(
				`${
					authors.post.author.nickname ?? authors.post.author.name
				} mentioned you`,
				`${authors.post.content}`,
				{ type: "post", id: authors.post.id },
				mentionedPeople,
				"mention"
			);
			await sendNotification(
